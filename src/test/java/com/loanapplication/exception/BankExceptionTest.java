...
public class BankExceptionTest {
	...
	@Test
	public void testUpdateBankInvalidDataException() throws Exception {
		BankDTO bankDTO = new BankDTO(); // Create an invalid BankDTO

		RequestBuilder requestBuilder = MockMvcRequestBuilders.put("/api/banks/1")
				.content(MasterData.asJsonString(bankDTO)).contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON);
	...
	@Test
	public void testUpdateBankByIdNotFoundException() throws Exception {
		Long bankId = 100L;
		ErrorResponse exResponse = new ErrorResponse(HttpStatus.NOT_FOUND.value(), "Bank not found");

		when(this.bankService.updateBank(bankId, new BankDTO())).thenThrow(new NotFoundException("Bank not found"));
		RequestBuilder requestBuilder = MockMvcRequestBuilders.put("/api/banks/" + bankId)
				.content(MasterData.asJsonString(new BankDTO())).contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON);

		MvcResult result = mockMvc.perform(requestBuilder).andReturn();

		yakshaAssert(currentTest(), result.getResponse().getStatus() == 400, exceptionTestFile);
	}

	@Test
	public void testDeleteBankByIdNotFoundException() throws Exception {
		Long bankId = 100L;
		ErrorResponse exResponse = new ErrorResponse(HttpStatus.NOT_FOUND.value(), "Bank not found");

		doThrow(new NotFoundException("Bank not found")).when(this.bankService).deleteBank(bankId);
