...
public class LoanExceptionTest {
	...
	@Test
	public void testGetLoanByIdNotFoundException() throws Exception {
		Long loanId = 100L;
		ErrorResponse exResponse = new ErrorResponse(HttpStatus.NOT_FOUND.value(), "Loan not found");

		when(this.loanService.getLoanById(loanId)).thenThrow(new NotFoundException("Loan not found"));
		RequestBuilder requestBuilder = MockMvcRequestBuilders.get("/api/loans/" + loanId)
				.content(MasterData.asJsonString(new LoanDTO())).contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON);
	...
	@Test
	public void testUpdateLoanByIdNotFoundException() throws Exception {
		Long loanId = 100L;
		ErrorResponse exResponse = new ErrorResponse(HttpStatus.NOT_FOUND.value(), "Loan not found");

		when(this.loanService.updateLoanStatus(loanId, "false")).thenThrow(new NotFoundException("Loan not found"));
		RequestBuilder requestBuilder = MockMvcRequestBuilders.put("/api/loans/" + loanId)
				.content(MasterData.asJsonString(new LoanDTO())).contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON);
